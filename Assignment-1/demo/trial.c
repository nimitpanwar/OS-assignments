...
void load_and_run_elf(char** exe) {
    ...
    // Calculate the address where the loadable segment content starts
    ...
    free(content); // Free the memory allocated for ELF content
    free(phdr); // Free the memory allocated for program headers
}
